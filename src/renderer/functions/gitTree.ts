...
export const parseBranchNamesBelow = async (hash: string, workingDir?: string) => {
    ...
    const branches: string[] = splitString(branchesWithRefs.slice(
        0,
        branchesWithRefs.indexOf("remotes")
    ),"\n")

    for (let i = 0 ; i < branches.length ; i += 1) {
        let thisBranchName = branches[i]
        if (thisBranchName.indexOf("*") === -1) {
            thisBranchName = thisBranchName.slice(
                thisBranchName.indexOf(" ")+2
            )
        }
        else {
            thisBranchName = thisBranchName.slice(
                thisBranchName.indexOf(" ")+1
            )
        }
...
export const generateTreeMetadataHierarchy = (
    hashList: string[], 
    ...
    for (let k = hashList.length-1; k >= 0 ; k -= 1) {
        ...
        if (!isInitial) {
            ...
            const thisCommitParentHashes: string[] = splitString(hashesString," ")
...
export const parseLogTree = async (workingDir?: string) => {
    ...
    const logListString = await Promise.resolve(git.raw([
        "log",
        "--full-history",
        "--topo-order",
        "--format=%H%%%%%an%%%%%s%%%%%ad%%%%"
    ]))
    const logListSplitted = splitString(logListString,"\n")
    for (let i = 0 ; i < logListSplitted.length ; i += 1 ) {
        const logListSplittedElement = splitString(logListSplitted[i],"%%",1)
        logList.push({
    ...
    const parentHashListString = await Promise.resolve(git.raw([
        "log",
        "--full-history",
        "--topo-order",
        "--format=%P"
    ...
    const hashListString = await Promise.resolve(git.raw([
        "log",
        "--full-history",
        "--topo-order",
        "--format=%H"
    ...
    const hashList: string[] = splitString(hashListString,"\n")

    const parentHashList: string[] = splitString(parentHashListString,"\n")
    ...
    for (let i = 0; i < logList.length ; i += 1) {
        ...
        const branchesBelow = await parseBranchNamesBelow(
            logList[i].hash,
            data.workingDir
        )
    ...
    const buildCausalityTree = () => {
        ...
        const fillNodeData = (nodeNumber: number) => {
            const branch = {
                ...
                branchColor: branchesData[branchesList.indexOf(
                    fullHistory[nodeNumber].branch
                )].branchColor
            }
                ...
                if (!currentNodeMetadata.isLeaf) {
                    ...
                    if (currentNodeMetadata.isMerge && !currentNodeMetadata.isDivergence) {
                        ...
                        const destinationBranch = {
                            ...
                            branchColor: branchesData[branchesList.indexOf(
                                fullHistory[currentNodeNumber].branch
                            )].branchColor
                        }
                    ...
                    if (!currentNodeMetadata.isDivergence && !currentNodeMetadata.isMerge) {
                        ...
                        if (metadataList[currentNodeMetadata.childrenOf[0]].isDivergence && !isFirstChild) {
